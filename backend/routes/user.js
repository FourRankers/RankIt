...
router.post("/:userId/vote-post/:postId", async (req, res) => {
  try {
    ...
    if (userVoteDoc.exists) {
      ...
      if (currentVote === vote) {
        ...
        return res.status(200).json({ 
          ...
          currentVotes: postDoc.data().upvotes - currentVote
        });
      }
      ...
      return res.status(200).json({ 
        ...
        currentVotes: postDoc.data().upvotes + (vote - currentVote)
      });
    }
    ...
    res.status(200).json({ 
      ...
      currentVotes: postDoc.data().upvotes + vote
    });
  } catch (error) {
